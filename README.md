# window — 都市の静けさを、そっと味わう時間。

静かな都市の風景を、ブラウザ上の「窓」として眺められる Web アプリです。雨の都会の夜、深夜の高層階、橋のオフィス街などの情景を、環境音とともに眺め、そのときの気持ちをひとことだけ記録できます。

中心にあるのは記録機能ではなく、**窓をひらいて眺める体験**です。

> 仮称 `window` は、設定 1 か所（`src/config/brand.ts` または環境変数）で変更できます。→ [アプリ名の変更](#アプリ名の変更)

---

## コンセプト

- 高級感がある／静か／空気感がある／奥行きを感じる
- 暗いが見づらくない、グラスモーフィズムが背景と自然に馴染む
- ネイティブアプリの模倣ではなく、ブラウザだから成立する Web 体験
- シーンごとの URL・リンク共有・キーボード操作・ブラウザの戻る/進む
